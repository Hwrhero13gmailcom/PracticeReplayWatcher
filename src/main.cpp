...
static std::string FindRecentPracticeReplay(std::string const& mapId) {
    ...
    for (auto const& entry : std::filesystem::directory_iterator(replayDir)) {
        ...
        // Only files written in the last 30 seconds
...
ON_EVENT(MetaCore::Events::MapEnded) {
    ...
    // Wait for BeatLeader to finish writing, then check for a recent practice file
    BSML::MainThreadScheduler::ScheduleAfterTime(1.5f, [mapId]() {
        ...
        // Show a floating screen with Watch / Dismiss buttons
        ...
        closeBtn->GetComponent<UnityEngine::RectTransform*>()->set_anchoredPosition({18.0f, 0.0f});
